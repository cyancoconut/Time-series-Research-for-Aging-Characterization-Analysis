...
failure is instructive because it is not a failure of segmentation or of
features: the UConn-ILCC records contain, per cell, \num{37} discharges within
...
correctly cut and correctly described --- of which the clustering labels three.
Density-based clustering assigns labels by the composition of the population it
is given, and a foreign laboratory's mix of segment types is not the one its
thresholds were set against. The configured criterion, which asks only whether a
segment is a full discharge at a declared rate, is indifferent to that mix.
