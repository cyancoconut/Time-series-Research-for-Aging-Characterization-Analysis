...
class DismemblerFunctions:
    ...
    def dismembling(self, df_cell):
        ...
        for programm_name, programm_df in df_cell.groupby("BM_Programm"):
            ...
            # set BM_Programm_procedure=0 for every row that is PAU, except the procedure includes "_EIS_"
            # Set target="EIS*" for PAU rows that have "_EIS_" in Prozedur
            ...
            # Set BM_Programm_procedure=0 for PAU rows that don't have "_EIS_" in Prozedur
            programm_df.loc[mask_pau & ~mask_eis, "BM_Programm_procedure"] = 0

            # Check if the procedure is too short
