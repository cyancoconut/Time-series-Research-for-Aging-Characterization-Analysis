...
- Tab 2 → `download/build_bronze_cu_with_ah.py <battery_cfg> [--cells …] [--overwrite] [--incremental]`. Honors `download_from`/`upload_to` (same as `main.py`); `local` reads per-test parquets from `<working_path>/<cell>/*.parquet`, `minio` (default) from `<minio_prefix>/<cell>/`. Legacy `save_local`/`upload_s3` accepted when `upload_to` absent. Skips if the target BRONZE_CU already exists. **`--incremental`** appends only test files not yet in the per-cell manifest sidecar (`<cell>_manifest.json`, beside BRONZE_CU local + MinIO-untagged) instead of re-reading/re-integrating every file: the manifest carries the processed-file list + running Ah total + last sample, so the cumulative Ah integral continues seamlessly and the output equals a full rebuild. **Dead time between test files books no Ah**: any interval whose Δt exceeds the gap cut (auto `50 × median Δt`, or a fixed `ah_gap_threshold_s`) is masked, so a parked cell between files can't interpolate phantom throughput across the gap (`util/add_ah_throughput.py`). **Nothing at all is booked *between* files**: a frame's first row has no predecessor within that frame, so the inter-file interval belongs to no integral — which is what makes an incremental build equal a full rebuild by construction. There is deliberately **no "bridge"** joining the manifest watermark to the first new sample; one existed until it was removed, a leftover from when the full path integrated the row *union*, and once integration went per-file it became the only term booking an inter-file seam — so it broke the very equality it was meant to preserve. It could not be masked either: a two-row frame gives `gap_cut` one interval and its `len(dt_s) > 2` guard returns `None`, so the park was booked in full and no `gap_factor` could have caught it. **Each test file is integrated on its own** (`_frame_contributions`), and only the per-sample contributions are merged onto one time axis and accumulated — integrating the *union* of the rows is wrong whenever two files overlap in wall-clock time, which a parametrization run does routinely (cycler test + a parallel EIS logger recording at zero current). The trapezoid interpolates current between consecutive samples, so an interleaved zero-current recording drags it to zero and back between every real pair; at equal cadence that **halves** the integral. Measured on `J8049_Namey_NFPP_28Ah_02` before the fix: a 1 s cycler segment booked 0.6932 Ah against a true 1.3857 Ah (ratio 1.999), 0.100 s pulse segments were off by 1.05 (fewer foreign rows per real interval — the cadence dependence is the fingerprint), and the cell total read 274.30 Ah against 390.92 Ah. Summing per-file integrals is also the physically right answer: each instrument's charge is booked once, at the timestamps where it flowed, and a passive logger contributes nothing. Two *current-carrying* files that overlap would be double-counted instead, which should never happen (one cell, one channel) and prints a warning. Full builds also write the manifest; `--incremental` falls back to a full build when no prior state exists. **Still-running tests** (`…=unfinished.parquet`) are excluded from BRONZE_CU and from the committed manifest state in every mode (`_is_unfinished`) — a partial export can read back with an object-dtype `Zeit`, and it re-downloads under a *different* basename once finished, which the basename-keyed manifest would double-count. Their tail is instead recorded under the manifest's own **`unfinished`** block (`_unfinished_state`: `files`, `ah_total`, `last_zeit`, `last_strom`, `last_prozedur`), so the aging monitor can show what a cell is doing right now. The provisional `ah_total` is offset by the committed one (the same carry-forward `--incremental` uses), so it is on the same scale as the real number — verified equal to the committed total once the test finishes. It is recomputed from scratch on every build, never read back by `_load_incremental_state`, and disappears when the file lands under its final name; anything malformed in a partial export costs the provisional value, never the build. A build with no *new finished* files still rewrites the manifest to refresh the block (committed fields untouched) — otherwise the common case, a cell whose only new data is the running test, would never refresh. Assumes new files have later timestamps (a backfilled earlier-`Zeit` file needs `--overwrite`). CU detection (`_is_cu`): per-test file is a check-up when its 4th `=`-delimited filename field contains `procedure_filter`, which is **required** (`process_cell` raises `ValueError` if unset). `procedure_filter` may be a single substring or a **list of substrings** (matches when the field contains **any** entry — e.g. `["jri_CU", "jri_Char"]` to fold a characterization run in with the routine checkup); normalized via `util.procedure_filter.as_filter_list`.
...
1. **`dismember/dismember_raw_cell.py`** — renames German columns (`Spannung→Voltage`, `Strom→Current`, `Zeit→Time`, `T1→Temperature`), segments into procedures. Groups by `Ahjo_Test_ID` → `BM_Programm`, splits on `Prozedur` changes and PAU pauses > `pau_duration`. Drops segments < `min_rows`. ID = `<BM_Programm>_<procedure_number>` (e.g. `13_16`). Core: `dismember/cluster_preparation.py`.
   ...
   - **Only a *real* EIS procedure has its pauses relabeled**: PAU rows become `Zustand="EIS"` when the `Prozedur` contains `_EIS_` **and** that procedure carries no charge-moving `Zustand` (`CHA`/`DCH`) anywhere — a real EIS procedure only rests (`PDE_MEC_STD_EIS_diga` is 89 k rows of pure PAU), while a cycler procedure may merely mention EIS in its name while doing the pulsing itself (`csi_Hina_Puls_EIS_neg10`, `zho_CHAR_Pulse_EIS_CHA`). The name alone used to be enough, which had two consequences for such a procedure: its ordinary rests left `PAU_Columns` and so stopped cutting a **procedure boundary** (the long-PAU rule tests the post-relabel mask), and they acquired an `EIS` target they had not earned, so `util/soc_from_steps.py` read them as EIS measurement blocks. On `J8041_Hina_10Ah_19` / `csi_Hina_Puls_EIS_neg10` both bit at once: one 21 h segment (`23_1`, 76 k rows) held a 5.103 Ah emptying discharge, a 9.362 Ah full charge and the 8.917 Ah **capacity** discharge behind 240/30/15/255 min rests — all ≫ `pau_duration` 9.9 — whose signed currents averaged to −0.22 A (C/48), so the segment read as a rest, no CAP was labelled and every bundle exported `…_NASOH.parquet` for `fit_characterization` to skip. It splits into 7 segments with the gate, the capacity discharge standing alone at −0.182 C, and the SOC ladder reads 0 → 100 % in exact 5.00 % steps instead of **−52.26 … 47.77 %** (the three capacity legs had been booked as SOC steps of −57/+105/−100 %). Note `J8041_Hina_10Ah_14`'s `csi_Hina_Puls_EIS` was never affected only because it lacks the trailing underscore of `_EIS_`; the gate makes the two consistent. Measured: `J8049_Namey_NFPP_28Ah_02` BRONZE_PARA 278 → 277 segments, its 20 `DCH|EIS` lumps in `zho_CHAR_Pulse_EIS_CHA` splitting into step + dwell stub, with BM22's 21-block SOC ladder **bit-identical** and all 21 spectra still matching a `PDE_MEC` window (on `J8041_Hina_10Ah_14` all 21 already matched `PDE_MEC`, never a `csi_Hina_Puls_EIS` dwell). A cell with no `_EIS_` procedure is unaffected (`J8041_Hina_10Ah_105` CU: byte-identical).
...
6. **`output/export_pulse.py`, `export_qocv.py`** — optional per-`BM_Programm` PUL / qOCV exports from GOLD (flags `export_pulse`, `export_qocv`, default off). Per program, capacity comes from that program's CAP segment; `SOH = round(Capacity_py / nom_capacity * 100, 1)` (or `NA` + warning if no CAP). Pulse export also bundles adjacent PAU stubs (proc_num ±1) for before/after relaxation voltage, **except proc 0** — `<BM>_0` is dismember's discard bucket, not a pause: it carries `target="PAU"` but spans the *whole* test, so a pulse at procedure 1 pulled it in as its "before" stub and `_rehydrate_pau` then sliced all of BRONZE_CU back into the bundle, duplicating every pulse's samples at identical timestamps (BRONZE raw vs GOLD values differ slightly, so the 2RC fit sees two voltages per instant). On `J8041_Hina_10Ah_14` BM4 that was 631 k bucket rows and 82 k duplicated timestamps: 20 of 32 fits came back with the bucket's `ID`, RC parameters railed at their bounds and rmse 55–700 mV. With proc 0 excluded the same bundle yields 35 fits, median rmse 1.7 mV, `R0` monotone in OCV, every `ID` matching a `PUL` row in `with_features_post_labeled`. Latent since the June rehydration change — it needs a pulse at procedure 1, and both NFPP sweeps open with charge/rest segments so their first pulse sits at proc 8–9. `select_pulse_segments` (both fitters) additionally requires a segment to contain a `target == "PUL"` row, since it re-derives segments from `Zustand` runs alone and cannot otherwise tell a pulse from a CC leg of the same amplitude; skipped when `target` is absent or holds no `PUL`, so unlabelled bundles still fit. Files:
...
- **Source** (`download_from`): `40_capacity_monitore/*_capacity.csv` for SOH/CU history, plus only the **last row group** of each **BRONZE_CU** parquet (`ParquetFile.read_row_group`) for the latest `Zeit`→`Time` and `Prozedur`. Reading BRONZE_CU (not GOLD) means a BRONZE_CU rebuild alone refreshes time/status; SOH refreshes only on a full pipeline run. MinIO uses `io_router.open_bronze_range` (range-read footer + last row group). `last_row_time`/`last_Prozedur` then take whichever is **newer**, that tail or the BRONZE_CU manifest's `unfinished` block (`_latest_activity` + `fetch_manifest`) — a still-running test is deliberately absent from BRONZE_CU, so on exactly the busy cells this monitor is for, the parquet tail predates the current test by up to a whole check-up. The block is only as fresh as the last BRONZE_CU build, so run Tab 2 before the monitor.
