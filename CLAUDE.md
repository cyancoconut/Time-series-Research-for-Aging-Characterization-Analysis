...
- **LLM labels as training targets** (`train_classifier --labels llm`, or config `classifier_label_source: "llm"`; default `"target"`): trains the classifier directly on the free-form `llm_label` column (from `interpret_clusters`), **not** the fixed pipeline taxonomy — the model learns the LLM vocabulary as-is (`full_discharge_c2`, `pulse`, `full_charge_c20`, `mixed_*`, …). Segments with no `llm_label` are dropped; no PUL*RES-merge / bootstrap (the LLM already merges restore/test pulses). `_meta.json["label_source"]` records which space the model predicts. At inference `predict_classifier._map_llm_label_to_tagged` maps the free-form prediction back: `*pulse*→PUL*`, `*qocv*→QOCV*` (legacy labels still honored), and a `full_charge`/`full_discharge` is resolved by its **measured** `abs_Current_mean` (not the label's crate suffix): `→QOCV*` when ≤ `qocv_crate × (1 + qocv_tol)` (default tol 0.2, charge or discharge), else `→CAP*` when a `full_discharge` within `cap_rate ± 5 %`. `qocv_crate ≪ cap_rate` so the bands never overlap; qocv is checked first. Every other label passes through into GOLD `target` informationally. `predict_targets` takes `cap_rate` and `qocv_rate` (passed from `main.py` as `cfg["cap_rate"]` / `cfg["qocv_crate"]`); with a rate unset, the corresponding tag is never produced.
