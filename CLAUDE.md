...
1. **`dismember/dismember_raw_cell.py`** — renames German columns (`Spannung→Voltage`, `Strom→Current`, `Zeit→Time`, `T1→Temperature`), segments into procedures. Groups by `Ahjo_Test_ID` → `BM_Programm`, splits on `Prozedur` changes and PAU pauses > `pau_duration`. Drops segments < `min_rows`. ID = `<BM_Programm>_<procedure_number>` (e.g. `13_16`). Core: `dismember/cluster_preparation.py`.
   ...
   - **A renamed pause still cuts a boundary**: PAU rows inside a procedure whose name contains `_EIS_` are relabeled `Zustand="EIS"` so the EIS dwell windows survive the stub reduction and stay available as EIS match anchors. That relabel used to run *before* the boundary rules read the PAU mask, so in such a procedure neither the long-PAU rule nor `prev_group_was_long_pau` could fire, and a procedure whose legs are separated only by renamed rests never split at all. `cluster_preparation.dismembling` now snapshots the pause mask as `is_pause` **before** the relabel and feeds the boundary rules from it; everything downstream (stub reduction, pure-PAU pre-labelling, the too-short exemption) keeps the post-relabel `mask_pau`, so EIS windows still keep all their rows and become their own `EIS` segments. Found on `J8041_Hina_10Ah_19`, procedure `csi_Hina_Puls_EIS_neg10`: one 21 h segment (`23_1`, 76 k rows) held a 5.103 Ah emptying discharge, a 9.362 Ah full charge and the 8.917 Ah **capacity** discharge, separated by 240/30/15/255 min rests — all four ≫ `pau_duration` 9.9. Its signed currents averaged to −0.22 A (C/48), so it read as a rest, no CAP was ever labelled, every bundle exported as `…_NASOH.parquet` and `fit_characterization` skipped all of them (empty `plots/`). It splits into 7 segments with the fix, the capacity discharge standing alone at −0.182 C. On `J8049_Namey_NFPP_28Ah_02` BRONZE_PARA, 258 of 278 segments are unchanged; the 20 that change are BM22's `DCH|EIS` lumps in `zho_CHAR_Pulse_EIS_CHA`, each splitting into its 29.7 min C/10 **SOC step** + its 240 min EIS dwell (row count and ΔAh preserved), which is what `util/soc_from_steps.py` wants anyway — the step behind each EIS block is now its own segment. Pulses, CAP and qOCV segments are untouched, but **BM22's procedure numbers renumber, so re-export its bundles** (`segment_ID` changes). A cell with no `_EIS_` procedure is provably unaffected (`J8041_Hina_10Ah_105` CU: byte-identical segment table).
