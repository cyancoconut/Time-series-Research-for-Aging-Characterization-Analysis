...
The field-data track (`src/field/`) **moved to the manuscript repository** on
2026-09-14, to `~/Documents/Project_METAbatt_paper/analysis/field/`, with its
28 commits of history preserved through a `git subtree` merge.

It moved because nothing in this pipeline imported it — the dependency check
was clean in both directions — and its only consumer is §6 of the paper
("Beyond check-ups: opportunistic capacity from field data"). Keeping it here
meant the numbers in that section could go stale whenever this repository
moved, with nothing able to detect it; that is exactly what happened to the
abstract's scatter figure before the manuscript was split out on 2026-09-10.

**Do not re-add it here, and do not carry it into a `J8005_BMWK_METABatt` PR.**
The rule now is: code the check-up pipeline calls lives here; code whose only
consumer is the manuscript lives with the manuscript.

What moved: `io_shiyunliu.py`, `sessions.py`, `cluster_sessions.py`,
`extract_capacity.py`, `benchmark_shiyunliu.py`, `plot_field.py`,
`ica_prototype.py`, and the legacy `io_rwth.py` / `segment.py` from the earlier
RWTH Aachen scope. See `analysis/README.md` in that repository.

The datasets are unaffected and belong to neither repository:
`<working_data>/field_data/shiyunliu_20ev/` (20 production EVs, Deng et al.
Applied Energy 339:120954), plus `rwth_aachen/` and `tum_uds/` from earlier
scopes.
