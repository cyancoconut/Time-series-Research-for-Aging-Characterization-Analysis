...
| `classifier_model_path` | Optional. Path to a trained RandomForest (`models/vtc_classifier.joblib`). When set, `_process_cell_inner` uses the learned classifier instead of HDBSCAN + `post_cluster_filter`. Omit (default) to keep the HDBSCAN path. |
