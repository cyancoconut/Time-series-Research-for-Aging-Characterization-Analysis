...
- **LLM labels as training targets** (`train_classifier --labels llm`, or config `classifier_label_source: "llm"`; default `"target"`): trains the classifier directly on the free-form `llm_label` column (from `interpret_clusters`), **not** the fixed pipeline taxonomy — the model learns the LLM vocabulary as-is (`full_discharge_c2`, `pulse`, `qocv_c20`, `mixed_*`, …). Segments with no `llm_label` are dropped; no PUL*RES-merge / bootstrap (the LLM already merges restore/test pulses and qOCV DCH/CHA). `_meta.json["label_source"]` records which space the model predicts. At inference `predict_classifier._map_llm_label_to_tagged` maps the free-form prediction back: `*pulse*→PUL*`, `*qocv*→QOCV*`, `*full_discharge*→CAP*` **only** when the segment's measured `abs_Current_mean` is within `cap_rate ± 5 %` (so a non-CAP-rate full discharge is *not* a capacity test); every other label passes through into GOLD `target` informationally. `predict_targets` takes `cap_rate` (passed from `main.py` as `cfg["cap_rate"]`); if unset, nothing is tagged CAP*.
