%% b_resistance_estimation.m
% All cells with a 1 in the "path_to_data/timeseries/cells.xlsx" will be 
% evaluated here.

function b_resistance_estimation( path_to_data,parallel,compression, csv_export)
    arguments
...
cell_names = string(T.Cell_Name(T.b_resistance_estimation == 1));
...
if parallel == 1
    parfor i = 1:length(cell_names_converted)
        ...
        resistance_estimation_function('cell_name',cell_names(i),'path_to_data',path_to_data,'compression',num2str(compression*1),'csv_export',num2str(csv_export*1));
...
for i = 1:length(cell_names_converted)
    if cell_names_converted(i) == 1
        T.b_resistance_estimation(T.Cell_Name == cell_names(i) ) = 0;
    end
end
T = sortrows(T);
