%% e_add_ah_and_wh_throughput.m



function e_add_ah_and_wh_throughput( path_to_data,parallel,compression, csv_export)
    arguments
...
cell_names = string(T.Cell_Name(T.e_add_ah_and_wh_throughput == 1));
...
if parallel == 1
    parfor i = 1:length(cell_names_converted)
        ...
        add_ah_and_wh_function(cell_names(i),path_to_data,compression,csv_export);
...
for i = 1:length(cell_names_converted)
    if cell_names_converted(i) == 1
        T.e_add_ah_and_wh_throughput(T.Cell_Name == cell_names(i) ) = 0;
    end
end
T = sortrows(T);
