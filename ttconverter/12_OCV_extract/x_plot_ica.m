function x_plot_ica(varargin)
    ...
    x_lim = [min(TT_ocv.Voltage),max(TT_ocv.Voltage)];
    y_lim = [99999999,-999999999];
    ...
    %                     check if subset is empty
                        ...
                        for measurement_index = 1:length(measurements)
                            ...
                            if not(isempty(capacity_charge))
                                capacity_charge_filtered = csaps(voltage_charge,capacity_charge,0.99999999,voltage_charge);
                            else
                                capacity_charge_filtered = capacity_charge;
                            end
                            if not(isempty(capacity_discharge)) 
                                capacity_discharge_filtered = csaps(voltage_discharge,capacity_discharge,0.99999999,voltage_discharge);
                            else
                                capacity_discharge_filtered = capacity_discharge;
                            end
                            
                            ica_charge = gradient(capacity_charge_filtered,voltage_charge);
                            if not(isempty(voltage_charge))
                                ica_charge_filtered = csaps(voltage_charge,ica_charge,0.9999999,voltage_charge);
                            else
                                ica_charge_filtered = ica_charge;
                            end
                            
                            ica_discharge = gradient( capacity_discharge_filtered,voltage_discharge);
                            if not(isempty(voltage_discharge))
                                ica_discharge_filtered = csaps(voltage_discharge,ica_discharge,0.9999999,voltage_discharge);
                                y_lim = [min(min(ica_discharge_filtered),y_lim(1)),max(max(ica_discharge_filtered),y_lim(2))];
                            else
                                ica_discharge_filtered = ica_discharge;
                            end

                            
                            
                            ica_discharge_filtered = - ica_discharge_filtered;
                            ...
                            plot(subp(2),voltage_discharge, ica_discharge_filtered, 'color', mycolor(colorIndex, :));
    
                            % charge: Voltage over capacity
                            set(subp(1),'NextPlot','add');
                            set(fig,'CurrentAxes',subp(1));
                            plot(subp(1),voltage_charge, ica_charge_filtered, 'color', mycolor(colorIndex, :));
                        end
                        ...
                        set(subp(2),'NextPlot','add');
                        set(fig,'CurrentAxes',subp(2));
                        xlabel('Voltage in V','Interpreter','latex','FontSize',FontSizePlots,'fontname',FontNamePlots);
                        ylabel('$\frac{\partial Q}{\partial V}$ in $\mathrm{\frac{Ah}{V}}$','Interpreter','latex','FontSize',FontSizePlots,'fontname',FontNamePlots);
