function x_plot_dva(varargin)
    ...
    variable_for_color = 'Ah_throughput'; % 'Time', 'Current', 'Voltage', 'Temperature', 'EIS_Frequency', 'EIS_Z_abs', 'EIS_Z_phase', 'Ah_throughput', 'EIS_measurement_id'
    ...
    for voltage_index = 1:size(Voltage_limits,1)
        for temperature_index = 1:size(Temperature_limits,1)
            for current_index = 1:size(Current_limits,1)
                for ah_lindex = 1:size(Ah_throughput_limits,1)
                    for capacity_lindex = 1:size(Capacity_limits,1)
                        for time_index = 1:size(Time_limits,1)
                            TT_ocv_sub = TT_ocv(    TT_ocv.Voltage          >       Voltage_limits(voltage_index,1)             &           TT_ocv.Voltage          <           Voltage_limits(voltage_index,2)             &...
                                                    ...
                                                    TT_ocv.Capacity    >       Capacity_limits(capacity_lindex,1)           &           TT_ocv.Capacity    <           Capacity_limits(capacity_lindex,2)           &...
        ...
        %                     check if subset is empty
                            ...
                            for measurement_index = 1:length(measurements)
                                ...
                                if length(voltage_charge) > 2
                                    pp = csaps(capacity_charge,voltage_charge,0.999999);
                                    voltage_charge_filtered = ppval(pp,capacity_charge);
                                else
                                    voltage_charge_filtered = voltage_charge;
                                end
                                if length(voltage_discharge) > 2
                                    pp = csaps(capacity_discharge,voltage_discharge,0.999999);
                                    voltage_discharge_filtered = ppval(pp,capacity_discharge);
                                else
                                    voltage_discharge_filtered = voltage_discharge;
                                end
                                if (length(voltage_charge_filtered) == length(capacity_charge)) && length(voltage_charge_filtered) >2
                                    dva_charge = gradient(voltage_charge_filtered, capacity_charge);
                                else
                                    dva_charge = nan;
                                    capacity_charge = nan;
                                end
                                
                                nan_values = isnan(dva_charge());
                                dva_charge(nan_values) = [];
                                capacity_charge(nan_values) = [];
    
                                if length(capacity_charge) > 2
                                    pp = csaps(capacity_charge,dva_charge,0.9999);
                                    dva_charge_filtered = ppval(pp,capacity_charge);
                                else
                                    dva_charge_filtered = dva_charge;
                                end
                                if (length(voltage_discharge_filtered) == length(capacity_discharge)) && length(voltage_discharge_filtered) >2
                                    dva_discharge = gradient(voltage_discharge_filtered, capacity_discharge);
                                else
                                    dva_discharge = nan;
                                    capacity_discharge = nan;
                                end
    
                                nan_values = isnan(dva_discharge());
                                dva_discharge(nan_values) = [];
                                capacity_discharge(nan_values) = [];
                                    
                                if length(dva_discharge) > 2
                                    pp = csaps(capacity_discharge,dva_discharge,0.9999);
                                    dva_discharge_filtered = ppval(pp,capacity_discharge);
                                else
                                    dva_discharge_filtered = dva_discharge;
                                end
                                
                                dva_discharge_filtered = - dva_discharge_filtered;
                                ...
                                plot(subp(2),capacity_discharge, dva_discharge_filtered, 'color', mycolor(colorIndex, :));
        
                                % charge: Voltage over capacity
                                set(subp(1),'NextPlot','add');
                                set(fig,'CurrentAxes',subp(1));
                                plot(subp(1),capacity_charge, dva_charge_filtered, 'color', mycolor(colorIndex, :));
                            end
                            ...
                            ylabel('$\frac{\partial V}{\partial Q}$ in $\mathrm{\frac{V}{Ah}}$','Interpreter','latex','FontSize',FontSizePlots,'fontname',FontNamePlots);
    ...
    %                         set(gca, 'XDir','reverse');
        ...
        %                     ylim(ocv_y_lim);
                            ...
                            file_name = strcat(replace(cell_name," ","_"),"_dva");
